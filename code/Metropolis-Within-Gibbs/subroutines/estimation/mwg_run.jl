...
function proposal_cholesky(Σ::Array{Float64, 2})
     Σ_sym = Symmetric(Σ);

     for jitter in (0.0, 1e-12, 1e-10, 1e-8, 1e-6, 1e-4)
          try
               return cholesky(Σ_sym + jitter * I).L;
          catch err
     ...
     error("Proposal covariance is not positive definite, even after diagonal jitter.");
end
...
function mwg_run(θ_ini_unb::Array{Float64,1}, par::ParSsm, h::Int64, par_ind::BoolParSsm, par_size::SizeParSsm,
     ...
     elseif any(.!isfinite.(mwg_const)) || any(mwg_const .<= 0)
          error("mwg_const entries must be finite and strictly positive.");
     elseif !isfinite(acc_target) || acc_target <= 0 || acc_target >= 1
          error("acc_target must be a probability strictly between 0 and 1.");
     ...
     function run_phase!(phase_name::String, n_draws::Int64, scale::Float64, Σ::Array{Float64, 2};
          ...
          for draw=1:n_draws
               ...
               accepted = false;
               if apriori_rejection[1] == 0 && isfinite(par_prop.logposterior)
