...
# ----------------------------------------------------------------------------------------------------------------------
     ...
     Z   = [ones(n) zeros(n) [zeros(3); ones(n-3)] zeros(n)]; # the common factors in z
     ...
     # Projections
     Z_ind[2:4, 1] .= true; # All        -> efficient gap (not on 5th row because of same loading as 4th row)
     # Z_ind[2:3, 2] .= true; # All        -> efficient gap (not on 5th row because of same loading as 4th row)
     # Z_ind[4, 4] = true; # Prices     -> flex gap (only 4th row like above and only 4th column because 3rd column loads with unity or \kappa)
     ...
     ind_trends     = [7; 10]; # GDP trend and EMPL trend state indexes
     ...
     T = cat(dims=[1,2], [T_c for i=1:2]..., [T_ct for i=1:5]...); # Ψe(cycle), Ψπ(cycle),y(cycle+trend), e(cycle+trend), u(cycle+trend), UoM(cycle+trend), SPF(cycle+trend)
     Q = cat(dims=[1,2], [T_c for i=1:2]..., [T_ct for i=1:5]...); # same structure for Q for now (will change when adding correlation between shocks)
     ...
     P̄¹    = cat(dims=[1,2], [P̄_c for i=1:2]..., [P̄_ct for i=1:5]...); 
     ...
     λ     = vcat([λ_c for i=1:2]..., [λ_ct for i=1:5]...);
     ρ     = copy(λ);
     λ_ind = λ .!= 0;
     ρ_ind = copy(λ_ind);
