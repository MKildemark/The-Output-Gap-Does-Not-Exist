...
# ----------------------------------------------------------------------------------------------------------------------
     ...
     Z   = [ones(n) [zeros(3); ones(3)] [zeros(3); ones(3)]]; # the common factors in z
     Z1a = kron(Matrix(I, 3, 3), [1, 1])';         # Idiosyncratic parts of y, e, u   (cycle plus trend)           # idio C, , idio trend
     Z1b = kron(Matrix(I, 2, 2), [1, 1])';         # Idiosyncratic parts of inflation expectations (cycle plus trend)     # idio C, , idio trend
     ...
     # Projections
     Z_ind[[2,3], [1]] .= true; # All        
   
     

     # Z_plus_ind and Z_minus_ind (not used here)
     Z_plus_ind = zeros(size(Z)) .!= 0;
     Z_plus_ind[[4,5],[1]] .= true; # All       
     Z_plus_ind[5,2] = true; # Expect.    -> PC cycle, t-2   # busines cycle lag 2 loads on expectations only
     ...
     # make T_ind true on the diagonal for idx 1,2,4,6,8,10,11,13. estiamte ar1 coefficients
     for i in [1,2,4,6,8,10,11,13]
          T_ind[i,i] = true;
     end
