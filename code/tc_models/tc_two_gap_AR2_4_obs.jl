...
# ----------------------------------------------------------------------------------------------------------------------
     ...
     Z   = [ones(n) zeros(4) [zeros(2); ones(2)] zeros(4) [zeros(2); ones(2)]]; # the common factors in z
     Z1a = kron(Matrix(I, 2, 2), [1, 0, 1])';         # Idiosyncratic parts of y, e, u   (cycle plus trend)           # idio C, idio C+, idio trend
     Z1b = kron(Matrix(I, 1, 1), [1, 0, 1])';         # Idiosyncratic parts of inflation expectations (cycle plus trend)     # idio C, idio C+, idio trend
     ...
     # Projections
     Z_ind[[2], [1]] .= true; # All        -> okun
   
     

     # Z_plus_ind and Z_minus_ind (
     Z_plus_ind = zeros(size(Z)) .!= 0;
     Z_plus_ind[[3,4],[1]] .= true; # All        ->phillips
     Z_plus_ind[4,3] = true; # Expect.    -> phillips
     ...
     T = cat(dims=[1,2], T_c, [T_ct for i=1:3]..., [T_c for i=1:1]..., [T_ct for i=1:1]...); # PC(cycle), EP(cycle+trend),y(cycle+trend), e(cycle+trend), u(cycle+trend), inf(cycle), inf^c(cycle), UoM(cycle+trend), SPF(cycle+trend)
     ...
     λ     = vcat([1; 0], [λ_ct for i=1:3]..., [λ_c for i=1:1]..., [λ_ct for i=1:1]...);
     ρ     = copy(λ);
     λ_ind = λ .!= 0;
     ρ_ind = copy(λ_ind);
