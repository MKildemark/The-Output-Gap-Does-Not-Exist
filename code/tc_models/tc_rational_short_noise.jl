...
# ----------------------------------------------------------------------------------------------------------------------
     ...
     Z   = [ones(n) zeros(n) zeros(n) zeros(n) [zeros(3); ones(3)]]; # the common factors in z
     Z1a = kron(Matrix(I, 3, 3), [1, 0, 1])';         # Idiosyncratic parts of y, e, u   (cycle plus trend)           # idio C, idio C+, idio trend
     # NOTE: Removed Z2 and Z1b - no idiosyncratic cycles for I? and inflation expectations
     # Pad Z1a to 6x9 by adding zeros for I? and inflation expectations (rows 4-6)
     Z1a_padded = [Z1a; zeros(3, size(Z1a, 2))];

     Z  = [Z Z1a_padded];
     R  = zeros(n, n);         # observation noise for I? and inflation expectations
     # Add white noise for observations 4-6 (I?, UoM, SPF)
     R[4,4] = 1.0;  # initial value for I? observation noise
     R[5,5] = 1.0;  # initial value for UoM observation noise
     R[6,6] = 1.0;  # initial value for SPF observation noise
     ...
     R_ind[4,4] = true;  # estimate observation noise variance for I?
     R_ind[5,5] = true;  # estimate observation noise variance for UoM
     R_ind[6,6] = true;  # estimate observation noise variance for SPF
     ...
     # State ordering: 1-2 (g^e), 3-4 (g^I?), 5 (I¯^I?), 6-8 (y), 9-11 (e), 12-14 (u)
     # NOTE: Removed states 15-16 (I?) and 17-22 (UoM, SPF)
     ...
     # States: 1-2 (g^e), 3-4 (g^I?), 5 (I¯^I?), 6-8 (y), 9-11 (e), 12-14 (u)
     ...
     # PI, structure matches T: 2*2 for cycles, 1*1 for I¯^I?, then idio blocks for y, e, u only
     ...
     # NOTE: Removed I? (15-16) and UoM/SPF (17-22) cycles
     ...
     # Structure: [g^e_I¯, g^e_0, g^I?_I¯, g^I?_0, I¯^I?_0, y_I¯, y_0, y_0, e_I¯, e_0, e_0, u_I¯, u_0, u_0]
