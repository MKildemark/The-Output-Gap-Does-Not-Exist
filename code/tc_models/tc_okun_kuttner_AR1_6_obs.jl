# Common and idiosyncratic infaltion trend
...
# ----------------------------------------------------------------------------------------------------------------------
     ...
     Z   = [ones(n) zeros(n) [zeros(3); ones(3)./σʸ[4:6]]]; # the common factors in z
     Z1a = kron(Matrix(I, 3, 3), [1, 1])';         # Idiosyncratic parts of y and u       # idio C, idio C+, idio trend
     Z1b = kron(Matrix(I, 2, 2), [1, 1])';         # Idiosyncratic parts of inflation expectations (cycle plus trend)     # idio C, idio C+, idio trend
     Z2  = kron(Matrix(I, 1, 1), [1])';            # idiosyncratic parts of infaltion (only idiosyncratic cycle)          # idio C, idio C+
     ...
     # Projections
     Z_ind[[2,3,4,5,6], [1]] .= true;  # estimate pi en Epi loadings on gap
     ...
     ind_trends     = [4,6]; # GDP trend has drift
     ...
     T = cat(dims=[1,2], [T_c for i=1:1]..., [T_ct for i=1:4]..., [T_c for i=1:1]..., [T_ct for i=1:2]...); 
     Q =  cat(dims=[1,2], [T_c for i=1:1]..., [T_ct for i=1:4]..., [T_c for i=1:1]..., [T_ct for i=1:2]...); 
     ...
     T_ind = zeros(size(T)) .== 1;  # all T_ind is zero. No coefficients to be estimated. They are set in λ_ind and ρ_ind below
      # make T_ind true on the diagonal for idx 1,3,5,7,9,10,12. estiamte ar1 coefficients
