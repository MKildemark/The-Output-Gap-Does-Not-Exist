...
# ----------------------------------------------------------------------------------------------------------------------
     ...
     Z   = [ones(n) zeros(n) [zeros(2); ones(2)./σʸ[3:4]]]; # the common factors in z
     Z1a = kron(Matrix(I, 2, 2), [1, 0, 1])';         # Idiosyncratic parts of y and u       # idio C, idio C+, idio trend
     ...
     T = cat(dims=[1,2], [T_ct for i=1:3]..., [T_c for i=1:1]..., [T_ct for i=1:1]...); 
     Q =  cat(dims=[1,2], [T_ct for i=1:3]..., [T_c for i=1:1]..., [T_ct for i=1:1]...); 
     ...
     P̄¹    = cat(dims=[1,2], [P̄_ct for i=1:3]..., [P̄_c for i=1:1]..., [P̄_ct for i=1:1]...); # trends have diffuse initial conditions
     ...
     λ     = vcat([λ_ct for i=1:3]..., [λ_c for i=1:1]..., [λ_ct for i=1:1]...);
     ρ     = copy(λ);
     λ_ind = λ .!= 0;
     ρ_ind = copy(λ_ind);
