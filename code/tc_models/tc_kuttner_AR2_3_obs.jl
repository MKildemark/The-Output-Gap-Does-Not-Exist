...
# ----------------------------------------------------------------------------------------------------------------------
     ...
     Z   = [ones(n) zeros(n) [zeros(1); ones(2)./σʸ[2:3]]];
     ...
     # Projections
     Z_ind[[2,3], [1]] .= true;  # estimate pi en Epi loadings on gap
     ...
     T = cat(dims=[1,2], [T_ct for i=1:2]..., [T_c for i=1:1]..., [T_ct for i=1:1]...); 
     Q =  cat(dims=[1,2], [T_ct for i=1:2]..., [T_c for i=1:1]..., [T_ct for i=1:1]...); 
     ...
     P̄¹    = cat(dims=[1,2], [P̄_ct for i=1:2]..., [P̄_c for i=1:1]..., [P̄_ct for i=1:1]...); # trends have diffuse initial conditions
     ...
     λ     = vcat([λ_ct for i=1:2]..., [λ_c for i=1:1]..., [λ_ct for i=1:1]...);
     ρ     = copy(λ);
     λ_ind = λ .!= 0;
     ρ_ind = copy(λ_ind);
